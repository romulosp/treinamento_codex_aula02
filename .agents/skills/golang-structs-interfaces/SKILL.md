...
description: 'Padrões de projeto de structs e interfaces em Go — composição, incorporação, asserções de tipo, switches de tipo, segregação de interfaces, injeção de dependência via interfaces, tags de campo de struct e receptores de ponteiro versus valor. Use essa habilidade ao projetar tipos em Go, definir ou implementar interfaces, incorporar structs ou interfaces, escrever asserções de tipo ou switches de tipo, adicionar tags de campo de struct para serialização JSON/YAML/DB ou escolher entre receptores de ponteiro e valor. Use também quando o usuário perguntar sobre "aceitar interfaces, retornar structs", verificações de interface em tempo de compilação ou compor interfaces pequenas em interfaces maiores.'
---

Padrões de projeto de structs e interfaces em Go — composição, incorporação, asserções de tipo, switches de tipo, segregação de interfaces, injeção de dependência via interfaces, tags de campo de struct e receptores de ponteiro versus valor. Use essa habilidade ao projetar tipos em Go, definir ou implementar interfaces, incorporar structs ou interfaces, escrever asserções de tipo ou switches de tipo, adicionar tags de campo de struct para serialização JSON/YAML/DB ou escolher entre receptores de ponteiro e valor. Use também quando o usuário perguntar sobre "aceitar interfaces, retornar structs", verificações de interface em tempo de compilação ou compor interfaces pequenas em interfaces maiores.
