---
name: golang-naming
description: 'Convenções de nomenclatura em Go (Golang) — abrange pacotes, construtores, structs, interfaces, constantes, enums, erros, booleanos, receptores, getters/setters, opções funcionais, acrônimos, funções de teste e nomes de subtestes. Use esta habilidade ao escrever código Go, revisar ou refatorar, escolher entre alternativas de nomenclatura (New vs NewTypeName, isConnected vs connected, ErrNotFound vs NotFoundError, StatusReady vs StatusUnknown no iota 0), debater nomes de pacotes Go (antipadrões de utilitários/helpers) ou perguntar sobre as melhores práticas de nomenclatura em Go. Também é acionada quando o usuário menciona MixedCaps vs snake_case, constantes em MAIÚSCULAS, prefixo Get em getters ou capitalização de strings de erro. NÃO use para perguntas gerais de implementação em Go que não envolvam decisões de nomenclatura.'
---
