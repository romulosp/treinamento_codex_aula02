...
description: 'Observabilidade diária em Go — os sinais sempre ativos em produção. Abrange registro estruturado com slog, métricas do Prometheus, rastreamento distribuído com OpenTelemetry, criação de perfis contínuos com pprof/Pyroscope, rastreamento de eventos RUM no servidor, alertas e painéis do Grafana. Aplique ao instrumentar serviços Go para monitoramento de produção, configurar métricas ou alertas, adicionar rastreamento OpenTelemetry, correlacionar logs com rastreamentos, migrar loggers legados (zap/logrus/zerolog) para slog, adicionar observabilidade a novos recursos ou implementar rastreamento compatível com GDPR/CCPA com Plataformas de Dados do Cliente (CDP). Não se destina a investigações de desempenho aprofundadas e temporárias (→ Consulte as habilidades golang-benchmark e golang-performance).'
---

Observabilidade diária em Go — os sinais sempre ativos em produção. Abrange registro estruturado com slog, métricas do Prometheus, rastreamento distribuído com OpenTelemetry, criação de perfis contínuos com pprof/Pyroscope, rastreamento de eventos RUM no servidor, alertas e painéis do Grafana. Aplique ao instrumentar serviços Go para monitoramento de produção, configurar métricas ou alertas, adicionar rastreamento OpenTelemetry, correlacionar logs com rastreamentos, migrar loggers legados (zap/logrus/zerolog) para slog, adicionar observabilidade a novos recursos ou implementar rastreamento compatível com GDPR/CCPA com Plataformas de Dados do Cliente (CDP). Não se destina a investigações de desempenho aprofundadas e temporárias (→ Consulte as habilidades golang-benchmark e golang-performance).
