---
name: golang-safety
description: 'Programação defensiva em Go para prevenir pânicos, corrupção silenciosa de dados e bugs sutis em tempo de execução. Use sempre que escrever ou revisar código Go que envolva tipos propensos a nulos (ponteiros, interfaces, mapas, slices, canais), conversões numéricas, ciclo de vida de recursos (defer em loops) ou cópia defensiva. Também é acionado em questões sobre pânicos de nulo, alias de anexação, acesso concorrente a mapas, comparação de ponto flutuante ou design de valor zero.'
---
