...
9. Planeje as verificações de qualidade, Sonar e cobertura configuradas no módulo, incluindo as evidências necessárias para confirmar os critérios da Definition of Done. Se não houver Sonar ou cobertura, planeje a Auditoria de Qualidade Assistida por LLM: build, tipo, lint e testes disponíveis; mapeamento de artefatos alterados para testes aplicáveis; revisão de defeitos, tratamento de erro, duplicação, código morto, complexidade e documentação; e registro completo em `validation.md`.
