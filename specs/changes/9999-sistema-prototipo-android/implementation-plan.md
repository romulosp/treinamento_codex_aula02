...
`PLANO_APROVADO_PARA_IMPLEMENTACAO`

## Fatos observados

- `proposal.md` e `spec.md` estão em `SPEC_APROVADA`, com ressubmissão formal sem pendência material.
- O diretório `apps/frontend/smartphone/sistema-prototipo-android/` ainda não existe; `apps/frontend/smartphone/` contém somente o README de organização.
...
- Android SDK, `adb`, Gradle Wrapper e caches Gradle não foram encontrados nos caminhos locais inspecionados.
- A configuração aprovada é: perfil `SIMPLE`, módulo `app`, `minSdk = 26`, `compileSdk = 36`, `targetSdk = 36`, Java 17, AGP 9.4.0, Kotlin 2.2.10, Gradle 9.6.0, Compose BOM estável `2026.06.01`, Activity Compose 1.13.0 e Lifecycle 2.11.0.
...
1. Criar a fundação Gradle/Android e validar identidade, SDKs, módulo, Java 17 e ausência de caminhos absolutos.
2. Implementar modelos imutáveis, catálogo das 212 variantes e regras puras de edição, seleção, calculadora e menu.
3. Implementar tema neutro e componentes Compose stateless em pastas próprias, com KDoc em português do Brasil.
4. Implementar ViewModel e tela-catálogo em vertical slice: composição expandida 800 x 600, recomposição compacta/média, insets, rolagem e preservação de estado.
5. Implementar o menu demonstrativo e integrar o fluxo local de confirmação/retorno.
6. Implementar testes junto a cada comportamento e atualizar `tasks.md` somente após evidência de conclusão.
7. Confirmar por hash que nenhum PNG/JPG/GIF/TTF de `source-material/` foi copiado ao projeto.
...
- Testes instrumentados/UI: `gradlew.bat connectedDebugAndroidTest`, condicionados a emulador API 26 e API 36 disponível.
